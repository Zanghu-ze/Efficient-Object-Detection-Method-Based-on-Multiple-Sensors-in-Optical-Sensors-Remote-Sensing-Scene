...
def run(data,
        ...
        workers=15,  # max dataloader workers (per RANK in DDP mode)
    ...
    if len(stats) and stats[0].any():
        ...
        mp, mr, map50, map55, map60, map65, map70, map75, map80, map85, map90, map95, map = p.mean(), r.mean(), ap50.mean(), ap55.mean(), ap60.mean(), ap65.mean(), ap70.mean(), ap75.mean(), ap80.mean(), ap85.mean(), ap90.mean(), ap95.mean(), ap.mean()
        nt = np.bincount(stats[3].astype(np.int64), minlength=nc)  # number of targets per class
        print('map@50:95:5', round(map50, 5), round(map55, 5), round(map60, 5), round(map65, 5), round(map70, 5), round(map75, 5), round(map80, 5), round(map85, 5), round(map90, 5), round(map95, 5), round(map, 5), round(mp, 5), round(mr, 5))
    else:
        nt = torch.zeros(1)
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            ...
            print('ap@50:95:5', round(ap50[i], 5), round(ap55[i], 5), round(ap60[i], 5), round(ap65[i], 5), round(ap70[i], 5), round(ap75[i], 5), round(ap80[i], 5), round(ap85[i], 5), round(ap90[i], 5), round(ap95[i], 5), round(ap[i], 5),  round(p[i], 5), round(r[i], 5))
